...
CKalman::CKalman(int nq0, int nr0)
{
	...
	Hk = CMat(nr,nq,0.0);
	Qt = Pmin = Xk = CVect(nq,0.0);  Pmax = FBTau = CVect(nq,INF);
	Rt = Zk = CVect(nr,0.0);  rts = CVect(nr,1.0);
	measflag = 0;
}

void CKalman::TimeUpdate(double kfts)
{
	CMat Fk;
	kftk += kfts;
	SetFt();
...
void CKalman::MeasUpdate(double fading)
{
	...
	for(int i=0; i<nr; i++)
	{
		if(measflag&(0x01<<i))
		{
			Hi = Hk.GetRow(i);
			Pxz = Pk*(~Hi);
			double Pzz = (Hi*Pxz)(0,0) + Rt(i)/rts(i);
			Kk = Pxz*(1.0/Pzz);
...
void CSINSKF::Update(const CVect3 *wm, const CVect3 *vm, int nSamples, double tk)
{
	sins.Update(wm, vm, nSamples, tk);
...
{
	double afa, hur10=36000, *peb=&sins.eb.i,*pebMax=&sins.ebMax.i,*pdb=&sins.db.i,*pdbMax=&sins.dbMax.i;
	CVect vtmp(nq,0.0);
	for(int i=0; i<15; i++)
	{
		if(FBTau.dd[i]<hur10)
		{
			afa = fbts<FBTau.dd[i] ? fbts/FBTau.dd[i] : 1.0;
			if(i<9)
			{
				vtmp.dd[i] = Xk.dd[i]*afa;  Xk.dd[i] *= 1-afa;
			}
			else if(i<12)
			{
				if((*peb>*pebMax&&Xk.dd[i]<0) || (*peb<-*pebMax&&Xk.dd[i]>0) || (*peb>=-*pebMax&&*peb<=*pebMax)) 
				{	*peb += Xk.dd[i]*afa;  Xk.dd[i] *= 1-afa; }
				peb++, pebMax++;
			}
			else if(i<15)
			{
				if((*pdb>*pdbMax&&Xk.dd[i]<0) || (*pdb<-*pdbMax&&Xk.dd[i]>0) || (*pdb>=-*pdbMax&&*pdb<=*pdbMax)) 
				{	*pdb += Xk.dd[i]*afa;  Xk.dd[i] *= 1-afa; }
				pdb++, pdbMax++;
			}
		}
	}
	sins.qnb -= *(CVect3*)&vtmp.dd[0];  sins.vn -= *(CVect3*)&vtmp.dd[3];  sins.pos -= *(CVect3*)&vtmp.dd[6];
}
...
void CSINSTDKF::TDUpdate(CVect3 *wm, CVect3 *vm, int nSamples, double tk, int nStep)
{
	sins.Update(wm, vm, nSamples, tk);
	Feedback(sins.nts);
...
CSINS::CSINS(const CQuat &qnb0, const CVect3 &vn0, const CVect3 &pos0, double tk0)
{
	...
	Kg = Ka = I33; eb = db = _tauGyro = _tauAcc = O31; ebMax = I31*100*glv.dph; dbMax = I31*10*glv.mg;
	wib = fb = fn = an = wnb = web = O31;
	etm(); lever();
}

void CSINS::SetedMax(CVect3 &e, CVect3 &d)
{
	ebMax = e; dbMax = d;
}
...
{
	_tauGyro.i = tauG.i>35999 ? 0.0 : 1.0/tauG.i;   // Gyro&Acc inverse correlation time for AR(1) model
	_tauGyro.j = tauG.j>35999 ? 0.0 : 1.0/tauG.j;
	_tauGyro.k = tauG.k>35999 ? 0.0 : 1.0/tauG.k;
	_tauAcc.i  = tauA.i>35999 ? 0.0 : 1.0/tauA.i;
	_tauAcc.j  = tauA.j>35999 ? 0.0 : 1.0/tauA.j;
	_tauAcc.k  = tauA.k>35999 ? 0.0 : 1.0/tauA.k;
}

void CSINS::Update(const CVect3 *wm, const CVect3 *vm, int nSamples, double tk0)
{
	nts = tk0-tk;	tk = tk0;
	double nts2 = nts/2;
...
/******************************  File Read or Write *********************************/
#ifdef IO_FILE_SURPORT

char CFileRdWt::dir[256] = {0};

CFileRdWt::CFileRdWt(char *dir0)  // set dir
{
	f = 0;
	int len = strlen(dir0);
	memcpy(dir, dir0, len);
	if(dir[len-1]!='\\') { dir[len]='\\'; dir[len+1]='\0'; }
}

CFileRdWt::CFileRdWt(char *fname0, int columns)
{
	char fname[256]={0};
	int findc=0, len0=strlen(fname0);
	for(int i=0; i<len0; i++)	{ if(fname0[i]=='\\') { findc=1; break; } }
	if(dir[0]!=0&&findc==0)	{ strcat(fname, dir); }
	strcat(fname, fname0);
